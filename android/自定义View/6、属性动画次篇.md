...
# <p id="jump3" />三、高级使用
## <p id="jump3_1" />AnimatorSet 多个动画结合，可以控制执行顺序
```java
...
## <p id="jump3_2" /> 二、PropertyValuesHolder.ofXXX()和ObjectAnimator.ofPropertyValuesHolder() 多个动画一起执行
```java
...
## <p id="jump3_3" />、Keyframe.ofXXX() + PropertyValuesHolders.ofKeyframe() + ObjectAnimator.ofPropertyValuesHolder()把同一个属性拆分
