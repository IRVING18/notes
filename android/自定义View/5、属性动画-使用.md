...
- [二、ObjectAnimator 自动赋值给对象；依赖set()/get()方法](#jump2)
  - [1、ObjectAnimator.ofInt/Float..() 系统自带基础属性动画]()
  - [2、ObjectAnimator.ofPropertyValuesHolder() + PropertyValuesHolder.ofXXX() 多个属性动画组合]() 
  - [3、ObjectAnimator.ofPropertyValuesHolder() + PropertyValuesHolders.ofKeyframe() + Keyframe.ofXXX() 把同一个属性拆分]()
  - [4、ObjectAnimator.ofObject(view,"",new xxxEvaluator(),start,end) 使用自定义估值器]()
